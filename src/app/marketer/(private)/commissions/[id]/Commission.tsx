...
export default function Commission({ order }: { order: any }) {
    ...
    <div className="p-4 space-y-6 font-dana">
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-lg">
          ...
          <div>
            وضعیت:
            <span className="px-2 py-1 text-sm rounded-full font-pelak bg-green-100 text-green-800">
              {order.status}
            </span>
          </div>
          <div>تاریخ ثبت: {formatShamsiDateString(order.created_at)}</div>
          <div>مبلغ کل: {order.total_amount.toLocaleString()} تومان</div>
          <div>کمیسیون: {order.commission.toLocaleString()} تومان</div>
        </div>
