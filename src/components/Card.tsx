...
function Card({ item }: { item: ProductType }) {
  return (
    <div className="bg-white w-full h-full border-gray-300 pb-5 pt-0 rounded-2xl flex flex-col gap-4 items-center group/item border ">
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="flex flex-col items-center">
          <p className="line-clamp-1 text-[#444444] text-[10px]">
            {item.name}
          </p>
          {item.originalPrice && (
          ...
          <p
            className={`text-lg ${item.originalPrice ? "text-red-500" : ""}`}
          >
            {item.currentPrice.toLocaleString("fa-IR")} تومان
          </p>
        </div>
        <button className="bg-primary-500 text-white px-7 py-1.5 rounded-full  hover:bg-primary transition-colors ease-in-out">
          افزودن به سبد خرید
        </button>
      </div>
    </div>
